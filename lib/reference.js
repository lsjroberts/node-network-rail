...
Reference.prototype.parseReferenceFile = function(lineCallback) {
    ...
    rd.on('line', function(line) {
        ...
        if (parsedLine.osEasting && parsedLine.osNorthing) {
            point = new OSPoint(parsedLine.osNorthing, parsedLine.osEasting);
            latlon = point.toWGS84();
            parsedLine.latitude = latlon.latitude;
            parsedLine.longitude = latlon.longitude;
        }

        if ('stanox' in parsedLine && parsedLine.stanox && parsedLine.stanox.length > 0) {
            var corpus = self.findCorpusLocationByStanox(parsedLine.stanox);
            if (corpus) {
                parsedLine.alpha3 = corpus['3ALPHA'];

                if (parsedLine.alpha3.trim().length == 0) {
                    parsedLine.alpha3 = null;
                }
            }
        }
